...
public class ShoppingOrderFragment extends BaseFragment {
    ...
    ModifyOrderParams buildOrderParams(HistoryOrderEntry entry) {
        ...
        params.setParams(entry.id, entry.orderno, entry.item_type, entry.combo_id, entry.price, entry.combo_idx, entry.status, entry.date, entry.name, entry.mobile, entry.address, entry.type,entry.placeAnOrderDate);
        return params;
    }
