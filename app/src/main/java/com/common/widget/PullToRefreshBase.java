...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	public static final int SMOOTH_SCROLL_DURATION_MS = 250;// defatult 200
	public static final int SMOOTH_SCROLL_LONG_DURATION_MS = 1325;
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			final float yDiff = y - mInitialY; // yDiff less than 0.0 implies
												// scrolling down while yDiff
												// greater than 0.0 implies
												// scrolling up. If I try to add
												// the less than or greater than
												// symbols, the preview refuses
												// to display it.
			if (mScrollDirectionListener != null) {
				if (yDiff < 0.0) {
					mScrollDirectionListener.onScrollingUp();
				} else if (yDiff > 0.0) {
					mScrollDirectionListener.onScrollingDown();
				}
			}
			break;
		}
	...
	/**
	 * Used internally for {@link #}. Allows derivative classes to include any
	...
	private void pullEvent() {
		...
		case PULL_FROM_END:
			newScrollValue = Math.round(Math.max(initialMotionValue
					- lastMotionValue, 0)
					/ FRICTION);
			itemDimension = getFooterSize() + 10;// cong default is 0
			break;
		case PULL_FROM_START:
		default:
			newScrollValue = Math.round(Math.min(initialMotionValue
					- lastMotionValue, 0)
					/ FRICTION);
			itemDimension = getHeaderSize() + 100;// cong
													// ???100使�?��????��??�???�度????????��????��??以�?�线
			break;
		}
	...
	public void setScrollDirectionListener(
			ScrollDirectionListener mScrollDirectionListener) {
